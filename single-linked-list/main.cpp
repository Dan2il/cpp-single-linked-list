...
class BasicIterator {
 ...
 public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        std::vector<Type> buffer_values(values);
        for (int index = buffer_values.size() - 1; index >= 0; index--) {
            PushFront(buffer_values[index]);
        }
    }
    ...
    void Clear() noexcept {
        for ( ; ; ) {
            Node* next_node_search = head_.next_node;
            if (next_node_search != nullptr) {
                head_.next_node = next_node_search->next_node;
                delete next_node_search;
            } else {
...
void Test0() {
    {
        ...
        assert(empty_int_list.GetSize() == 0);
